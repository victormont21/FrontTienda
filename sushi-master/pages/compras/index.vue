  ...
  <div>
    ...
    <v-container>
      <el-table v-if="mListaCompras.length > 0" :data="mListaCompras">
        ...
        <el-table-column min-width="150" label="ACCIONES">
          <template slot-scope="scope">
            <v-btn
    ...
    showOpenDialog(item) {
      ...
      for (var i = 0; i < datos.length; i++) {
        var arrys = [
          ...
          {
            text: Number(
              datos[i].quantity * parseFloat(datos[i].product.precio)
            ).toFixed(2),
            fontSize: 8.5,
            alignment: "center",
          },
        ...
        sumFalt =
          sumFalt + datos[i].quantity * parseFloat(datos[i].product.precio);
      }
      ...
      var docDefinition = {
        ...
        content: [
...

          {
            ...
            layout: "noBorders", // optional
            table: {
              body: [["TOTAL DINERO : " + Number(sumFalt).toFixed(2)]],
            },
          },
    ...
    async readComprasRealizadas() {
      try {
        ...
        var datos = await this.$axios.get(
          process.env.baseUrl + "/compra?email=" + email + "&fk_rol=2"
        );

        console.log(datos.data);

        this.mListaCompras.push(...datos.data);
      } catch (error) {
        this.mListaCompras = [];
      }
    },
